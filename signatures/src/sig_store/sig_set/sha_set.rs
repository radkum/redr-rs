...
use utils::{
    ...
    redr,
    sha256_utils::{Sha256Buff, sha256_from_path},
};

use super::{
    Description, SigName, SigSetTrait,
    signature::{SigBase, SigData, SigId, Signature},
    sigset_serializer::{BIN_CONFIG, SigSetSerializer},
};
...
impl SigSetTrait for ShaSet {
    fn append_signature(&mut self, _sig_id: SigId, sig_sha: Signature) {
        let description = sig_sha.description();
        let name = sig_sha.name();
        let SigData::Sha(sha) = sig_sha.data else {
            todo!()
        };

        self.sha_list.insert(sha.clone());
        self.sha_to_description.insert(sha.clone(), description);
        self.sha_to_name.insert(sha, name);
    }
    ...
    fn eval_file(
        ...
        let sha256 = utils::sha256_utils::sha256_from_file_pointer(file)?;
