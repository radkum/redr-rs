...
use shared::sha_buf::{Sha256Buff, convert_sha256_to_string, sha256_from_vec_of_vec};
use utils::{detection_report::DetectionReport, redr};
...
impl SigSetTrait for HeurSet {
    fn append_signature(&mut self, sig_id: SigId, sig: Signature) {
        ...
        log::trace!(
            ...
            imports
                .iter()
                .map(|i| convert_sha256_to_string(i))
                .collect::<Vec<_>>()
        );
        ...
        for import_sha in imports {
            ...
            match self.import_sha_to_import_index.get_mut(&import_sha) {
                None => {
                    self.import_sha_to_import_index
