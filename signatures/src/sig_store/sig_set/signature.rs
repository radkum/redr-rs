...
use shared::{
    event::{
        FileCreateEvent, image_load::ImageLoadEvent, process_create::ProcessCreateEvent,
        registry_set_value::RegistrySetValueEvent,
    },
    hasher::MemberHasher,
    sha_buf::{Sha256Buff, convert_sha256_to_string, sha256_from_sha_string, sha256_from_vec},
};
use utils::detection_report::DetectionReport;
...
impl Signature {
    pub(crate) fn from_yaml(yaml_sig: YamlSignature) -> Result<Self, SigSetError> {
        Ok(Self {
            base: yaml_sig.base,
            data: SigData::from_yaml(yaml_sig.data)?,
        })
    }
    ...
    pub(crate) fn new_heur(
        ...
        match sig_type {
            HeurSet::HEUR_MAGIC_U32 => Self {
                base: SigBase { name, description },
                data: SigData::Imports(imports),
            },
            HeurSet::DYN_MAGIC_U32 => Self {
                base: SigBase { name, description },
                data: SigData::Calls(imports),
            },
            HeurSet::BEH_MAGIC_U32 => Self {
                base: SigBase { name, description },
                data: SigData::Event(imports),
            },
