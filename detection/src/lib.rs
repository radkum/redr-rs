...
use console::Term;
use scanner::{ScanResult, Scanner, error::ScanError};
use shared::{
    cleaning_info::CleaningInfoTrait,
    constants::COMM_PORT_NAME,
    event::{
        Event, FileCreateEvent, get_event_type, image_load::ImageLoadEvent,
        process_create::ProcessCreateEvent, registry_set_value::RegistrySetValueEvent,
    },
    hasher::MemberHasher,
};
use signatures::sig_store::SignatureStore;
use utils::{redr, redr::MalwareInfo};
...
async fn process_message(
    ...
    Ok(match detection_report {
        ...
        Some(report) => ScanResult::Malicious(MalwareInfo::new(report.into())),
    })
}
